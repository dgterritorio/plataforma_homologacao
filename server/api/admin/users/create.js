...
async function insertRows(table, items, columns, pred, returning) {
    ...
    let ecolumns = columns.filter(col => col !== 'empresas').map(f => '"' + f + '"');

    let emailsToCheck = items.filter(r => r.hasOwnProperty('email')).map(r => r.email);

    let result;

    // Check if emails exists
    if (emailsToCheck.length) {
        result = await db.query('select email from webapp.users where email in (\'' + emailsToCheck.join("','") + '\')');
        ...
        if (len) {
            return { error: new VError(409, len > 1 ? 'Certos emails encontram-se em uso' : 'Email encontra-se em uso') };
        }
    }
    ...
    items.forEach(row => {
        let valuesClause = [];
        columns.filter(col => col !== 'empresas').forEach(c => {
    ...
    result = await db.query({ text: sql, values: params });

    return result;
};

export default async function (req, res, next) {
    console.log("> Request: Create Admin User");
    try {
