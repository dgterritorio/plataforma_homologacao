<template>
  <div>
    ...
    <div v-if="showSettings" class="row grid-x flex-nowrap mb-4">
      <!-- Disabled fields -->
      <div class="col">
        <div class="position-absolute pt-2 pl-3">
          {{ availableFieldsLabelText }}
        </div>
        <draggable
          v-model="internal.fields"
          class="d-flex flex-row drag-area has-label flex-wrap"
          :class="dragAreaClass"
    ...
    <div class="row flex-nowrap grid-x mb-4" v-if="showSettings">
      ...
      <div class="col">
        <draggable
          ...
          @start="_start"
          @end="_end"
          class="d-flex flex-row drag-area border-primary flex-wrap"
          :class="dragAreaClass"
        >
          <div v-for="field in internal.colFields" :key="'col-' + field.label">
            <div
              class="btn btn-draggable btn-primary btn-sm btn-block d-flex align-items-center"
              @click="_fieldClicked(field.label)"
            >
              <div class="btn-label text-truncate mr-2">{{ field.label }}</div>
              <div class="mdi mdi-drag" />
              <!-- <div
                class="sort-dict ml-auto"
                :class="{ '--desc': internal.fieldsOrder[field.label] === 'desc' }"
              />-->
            </div>
          </div>
        </draggable>
        <p
          v-if="internal.colFields.length === 0"
          class="drag-area-placeholder text-secondary"
        >
          {{ $t("Drag fields here") }}
        </p>
        <div
          v-else
          class="drag-area-clear-button circle-background bg-primary"
          v-b-tooltip:hover
          :title="$t('Clear column fields')"
          @click="_resetCols"
        ></div>
      </div>
    </div>
    ...
    <div class="row flex-nowrap grid-x position-relative">
      ...
      <div class="left-col" v-if="showSettings">
        ...
        <p
          v-if="internal.rowFields.length === 0"
          class="drag-area-placeholder text-secondary"
        >
          {{ $t("Drag fields here") }}
        </p>
      ...
      <div
        v-if="showSettings"
        class="table-option-button circle-background bg-primary dropdown-toggle"
      ...
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <a class="dropdown-item" href="#!" @click="_clickedSaveButton('csv')"
          >Save table in CSV</a
        >
        <a class="dropdown-item" href="#!" @click="_clickedSaveButton('tsv')"
          >Save table in TSV</a
        >
      </div>
    </div>
