<template>
  <div class="table-responsive">
    ...
    <div
      v-else-if="getNumOfCells() > numOfCellsLimitation"
      ...
      <table class="table table-hover table-sm">
        ...
        <thead>
          <tr
            ...
            :key="'header-col-' + colField.label"
            v-if="colField.showHeader === void 0 || colField.showHeader"
            class="bg-light"
          >
            <!-- Top left dead zone -->
            <th
              v-if="
                colFieldIndex === firstColFieldHeaderIndex && rowHeaderSize > 0
              "
              :colspan="rowHeaderSize"
              :rowspan="colHeaderSize"
              class="text-right font-weight-normal text-muted"
            >
              <span
            ...
            <th
              v-if="
                colFieldIndex === firstColFieldHeaderIndex && rowFooterSize > 0
              "
        ...
        <tbody>
          <tr v-for="(row, rowIndex) in rows" :key="JSON.stringify(row)">
            ...
            <th
              ...
              :key="'header-row-' + rowField.label"
              :rowspan="spanSize(rows, rowFieldIndex, rowIndex)"
              v-if="
                (rowField.showHeader === void 0 || rowField.showHeader) &&
            ...
            >
              ...
              />
              <template v-else>{{
                values[JSON.stringify({ col, row })]
              }}</template>
            </td>
            <!-- Row footers (if slots are provided) -->
            <th
              v-for="(rowField, rowFieldIndex) in rowFieldsReverse"
              :key="'footer-row-' + rowField.label"
              :rowspan="
                spanSize(rows, rowFields.length - rowFieldIndex - 1, rowIndex)
              "
              v-if="
                rowField.showFooter &&
                spanSize(
                  rows,
                  rowFields.length - 1 - rowFieldIndex,
                  rowIndex
                ) !== 0
              "
            >
    ...
    <div class="table-responsive-footer-wrapper">
      <table>
        <tfoot>
          <tr
            v-for="(colField, colFieldIndex) in colFieldsReverse"
            :key="'footer-col-' + colField.label"
            v-if="colField.showFooter"
          >
            <!-- Bottom left dead zone -->
            <th
              v-if="
                colFieldIndex === firstColFieldFooterIndex && rowHeaderSize > 0
              "
              :colspan="rowHeaderSize"
              :rowspan="colHeaderSize"
            ></th>
            ...
            <th
              ...
              :key="JSON.stringify(col)"
              :colspan="
                spanSize(cols, colFields.length - colFieldIndex - 1, colIndex)
              "
              v-if="
                spanSize(
                  cols,
                  colFields.length - colFieldIndex - 1,
                  colIndex
                ) !== 0
              "
            >
            ...
            <th
              v-if="
                colFieldIndex === firstColFieldFooterIndex && rowFooterSize > 0
              "
              :colspan="rowFooterSize"
              :rowspan="colFooterSize"
            ></th>
          ...
          <tr>
            <th :colspan="this.cols.length + this.rowFields.length">
...
export default {
  ...
  data() {
    return { reducer: this.setReducer() };
  },

  computed: {
    // Watched target properties for re-calculation
    calculationTriggers: function () {
      return [this.rowFields, this.colFields, this.reducer, this.data];
    },
    ...
    colHeaderSize: function () {
      return this.colFields.filter(
        (colField) => colField.showHeader === void 0 || colField.showHeader
      ).length;
    },
    ...
    rowHeaderSize: function () {
      return this.rowFields.filter(
        (rowField) => rowField.showHeader === void 0 || rowField.showHeader
      ).length;
    },
  ...
  methods: {
    setReducer: function () {
      let reducer = null;
      switch (this.reducerType) {
        case "sum":
          reducer = (acc, item, index, array) => {
            return acc + item[this.reducerField];
          };
          break;

        case "avg":
          reducer = (acc, item, index, array) => {
            return acc + item[this.reducerField] / array.length;
          };
          break;
    ...
    calculateCols: function () {
      ...
      const extractColsRecursive = (data, depth, filters) => {
        ...
        values.forEach((value) => {
          ...
          const filteredData = this.filterDataByValue({
            data,
            getter,
            filter: value,
          });
    ...
    computeValues: function () {
      ...
      this.rows.forEach((row) => {
        ...
        this.cols.forEach((col) => {
          ...
          const value = data.length > 0 ? data.reduce(this.reducer, 0) : "-";
...
.table-responsive-footer-wrapper table {
  width: 100%;
}

thead >>> th {
  /** not having this ruins the scrolling */
  border-top: none;
}
