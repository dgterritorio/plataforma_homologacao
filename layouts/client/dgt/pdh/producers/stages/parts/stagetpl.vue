...
import Introduction from "@/layouts/client/dgt/pdh/producers/stages/parts/introduction.vue";
import Conclusion from "@/layouts/client/dgt/pdh/producers/stages/parts/conclusion.vue";
...
export default {
  ...
  methods: {
    ...
    onSummary: async function (payloads) {
      ...
      try {
        const result = await this.$axios.post("/api/producer/state/predict", {
          requestId: requestId,
          advance: advance,
          email: email,
          password: password,
  ...
  computed: {
    ...
    lastObservations: function () {
      ...
      while (idx <= len && !foundObs) {
        ...
        observations = lastState.observations;
