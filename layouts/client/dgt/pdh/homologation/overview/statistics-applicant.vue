<template>
  <client-only>
    <v-row class="mt-5">
      <v-col sm="12" md="6" lg="6">
        <UserInfo
          :homologation="{
            ...
            new: $t('Request Homologation'),
            toManage: '/homologation/list',
            toNew: '/homologation/new',
          }"
        ></UserInfo>
        ...
        <!-- <v-card :height="300" class="mt-4">
          <v-card-title>{{ $t("Requests") }}</v-card-title>

          <v-card-text>
            <div
              class="requests unselectable text-center font-weight-bold text-h2 my-12"
            >
              {{ global.requests }}
            </div>
            ...
            >
              <span class="open unselectable font-weight-bold text-h6"
                >{{ $t("Open:") }} {{ global.open }}</span
              >
              <span class="font-weight-bold text-h6 px-2">/</span>
              <span class="finished unselectable font-weight-bold text-h6"
                >{{ $t("Finished:") }} {{ global.finished }}</span
              >
              <span class="font-weight-bold text-h6 px-2">/</span>
              <span class="cancelled unselectable font-weight-bold text-h6"
                >{{ $t("Cancelled:") }} {{ global.cancelled }}</span
              >
            </v-container>
          </v-card-text>
        </v-card> -->
      ...
      <v-col :sm="12" :md="6" :lg="6">
        <GlobalMap onlyUser height="570"></GlobalMap>
      </v-col>
      ...
      <v-col :sm="12" :md="6" :lg="6">
        <BasicTable
          ...
          :title="$t('Lista de Requerimentos de Homologação')"
          :sortBy="'start_date'"
          :sortDesc="true"
...
import GlobalMap from "@/layouts/client/dgt/pdh/homologation/details/global-map-private.vue";
...
export default {
  ...
  methods: {
    ...
    refresh: async function () {
      ...
      await this.requestStatistics(start, end);
    ...
    //       "/api/homologation/statistics/getuseractivity",
    //       {
    //         days: days,
    //       }
    //     );
    ...
    //     homologation.activity = this.genChartData(activityLabels, [
    //       {
    ...
    requestStatistics: async function (start, end) {
      const me = this;
      try {
